                ...
                localforage.getItem("queue").then(function(queue) {
                    if (queue === null) {
                        localforage.setItem("queue", []).then(function (queue) {
                            queue.push({form: "Patient Registration", display: vm.patient.person.names[0].familyName + ", " + vm.patient.person.names[0].givenName, url: "/openmrs/ws/rest/v1/patient", data: vm.patient});
